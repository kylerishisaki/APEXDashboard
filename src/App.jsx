...
function EditClientModal({client,onSave,onClose}){
  ...
  const s=k=>e=>setForm(f=>({...f,[k]:e.target.value}));
  return(<ModalWrap onClose={onClose}><div className="h2" style={{color:"var(--gold)",marginBottom:22}}>Edit Profile</div><div className="input-row"><div><label className="label">Name</label><input className="input" value={form.name} onChange={s("name")}/></div><div><label className="label">Title</label><input className="input" value={form.title} onChange={s("title")}/></div></div><div className="input-row"><div><label className="label">Start Date</label><input className="input" type="date" value={form.start_date} onChange={s("start_date")}/></div><div><label className="label">Phase</label><select className="input" value={form.phase} onChange={s("phase")}>{[1,2,3,4].map(p=><option key={p} value={p}>Phase {p} — {MACRO_PHASES[p-1].label}</option>)}</select></div></div><div className="field"><label className="label">Coach Note</label><textarea className="input" rows={3} value={form.coach_note} onChange={s("coach_note")}/></div><div className="field"><label className="label">Status</label><div style={{display:"flex",gap:8}}>{["active","inactive"].map(v=><button key={v} className={`btn btn-sm${form.status===v?" btn-gold":" btn-ghost"}`} onClick={()=>setForm(f=>({...f,status:v}))}>{v.charAt(0).toUpperCase()+v.slice(1)}</button>)}</div></div><div style={{display:"flex",gap:10,marginTop:6}}><button className="btn btn-gold" onClick={()=>onSave({name:form.name,title:form.title,phase:parseInt(form.phase),start_date:form.start_date,coach_note:form.coach_note,status:form.status})}>Save</button><button className="btn btn-ghost" onClick={onClose}>Cancel</button></div></ModalWrap>);
}
    ...
    <div className="fade-in">
      ...
      {selected.size>0&&!isClientView&&(
        <div style={{display:"flex",gap:10,alignItems:"center",flexWrap:"wrap",padding:"12px 16px",background:"rgba(139,124,246,.08)",border:"1px solid rgba(139,124,246,.25)",borderRadius:4,marginBottom:14}}>
          ...
          <button className="btn btn-teal btn-sm" onClick={async()=>{for(const id of selected)await updateAssignment(id,{done:true});await load();clearSelect();}}>✓ Mark Done</button>
          <button className="btn btn-ghost btn-sm" onClick={async()=>{for(const id of selected)await updateAssignment(id,{done:false});await load();clearSelect();}}>↺ Pending</button>
